<template>
  ...
  <UContainer class="min-h-screen mt-24">
    ...
    <section class="flex flex-col gap-6">
      ...
      <div>
        <UTimeline
          :default-value="project.timeline.length - 1"
...
let { data: response } = await useFetch(() => "/api/content/getprojects", {
...
const { data: mdprojects } = await useAsyncData("ideas", () =>
...
const { data: mehrdad } = await useAsyncData("somethingelse", () =>
  queryCollection("ideas")
    .where("title", "LIKE", String(route.params.slug))
    .all(),
);
</script>
