  ...
  <div class="relative min-h-screen py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-10">
    ...
    <div class="relative z-10 space-y-3 border-b border-zinc-200/80 dark:border-zinc-800/80 pb-8">
      ...
      <p class="text-sm sm:text-base text-zinc-600 dark:text-zinc-300 max-w-3xl leading-relaxed">
        Evidence-based clinical reviews, high-yield differential diagnosis breakdowns, and MCCQE1 / USMLE scenario
        analyses.
      </p>
    </div>
    ...
    <div class="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6">
      <!-- Search Tile (7 cols) -->
      <div
        class="lg:col-span-7 rounded-3xl p-6 sm:p-8 border border-zinc-200/80 dark:border-zinc-800/80 bg-white/60 dark:bg-zinc-900/50 backdrop-blur-xl shadow-xs space-y-4">
      ...
      <div
        class="lg:col-span-5 rounded-3xl p-6 sm:p-8 border border-zinc-200/80 dark:border-zinc-800/80 bg-white/60 dark:bg-zinc-900/50 backdrop-blur-xl shadow-xs space-y-4">
        <div class="flex items-center justify-between border-b border-zinc-200/80 dark:border-zinc-800/80 pb-3">
          <span class="text-xs font-bold text-zinc-900 dark:text-zinc-100 tracking-wider">Clinical Specialties</span>
        ...
        <div class="flex flex-wrap gap-2">
          <span v-for="cat in categories" :key="cat.name"
            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium bg-zinc-100 dark:bg-zinc-800/80 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700 hover:border-teal-500/40 transition-all cursor-pointer">
            <span>{{ cat.name }}</span>
    ...
    <div class="relative z-10 space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="item in articlesList" :key="item._id || item.slug"
          ...
          <div class="space-y-4">
            <div
              class="flex items-center justify-between gap-4 text-xs text-zinc-500 border-b border-zinc-200/80 dark:border-zinc-800/80 pb-3">
            ...
            <h3
              class="text-xl font-bold text-zinc-900 dark:text-zinc-100 group-hover:text-teal-700 dark:group-hover:text-teal-400 transition-colors leading-snug">
            ...
            <div v-if="item.sections && item.sections.length > 0"
              class="bg-zinc-50/80 dark:bg-zinc-950/40 p-4 rounded-2xl border border-zinc-200/50 dark:border-zinc-800/50 space-y-2">
              <span class="text-xs font-semibold text-zinc-700 dark:text-zinc-300 tracking-wider block">Key Case
                Modules:</span>
              <ul class="space-y-1.5 text-xs text-zinc-600 dark:text-zinc-400">
                <li v-for="(sec, idx) in item.sections.slice(0, 4)" :key="idx" class="flex items-start gap-1.5">
                  <UIcon name="i-solar-double-alt-arrow-right-linear"
                    class="size-3 text-teal-600 dark:text-teal-400 shrink-0 mt-0.5" />
          ...
          <div class="pt-4 border-t border-zinc-200/80 dark:border-zinc-800/80 flex items-center justify-between mt-6">
            <div class="flex items-center gap-2 text-xs text-zinc-500">
              ...
              <span>Mehrdad Anvar, MD, LMCC</span>
            </div>
